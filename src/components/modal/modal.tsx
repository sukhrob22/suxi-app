...
const Modal = () => {
  ...
  return (
    < MuiModal open={modal} onClose={handleClose} className={`fixed !top-7 left-0 right-0 z-50 mx-auto w-full max-w-5xl overflow-hidden overflow-y-scroll scrollbar-hide`} >
      <>
        <button onClick={() => setModal(false)} className='modalButton absolute right-1 top-3 !z-40 h-9 w-9 border-none bg-[#181818]'>
          <FaTimes />
        </button>

        <div className='relative pt-[55%]'>
          <ReactPlayer url={`https://www.youtube.com/watch?v=${trailer}`} width={'100%'} height={'100%'}
            playing={playing}
